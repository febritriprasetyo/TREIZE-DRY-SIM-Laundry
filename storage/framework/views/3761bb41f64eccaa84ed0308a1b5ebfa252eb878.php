                	...
                	<form method="POST" enctype="multipart/form-data" class="form_edit_identitas">
	                	...
	                	<div class="row">
	                		<div class="col-md-4 d-flex justify-content-start">
	                			<div class="foto">
	                				<img src="<?php echo e(asset('/pictures/' . auth()->user()->avatar)); ?>" class="foto-profile rounded">
	                			...
	                			<div class="identitas ml-4">
	                				<p class="text-dark font-weight-bold ubah_nama_text" style="font-size: 18px;"><?php echo e(auth()->user()->name); ?></p>
	                				<input type="text" name="ubah_nama_input" class="ubah_nama_input mb-3" value="<?php echo e(auth()->user()->name); ?>" hidden="">
	                				<p class="text-dark font-weight-bold" style="font-size: 12px; margin-top: -5px;">Kode pengguna, <?php echo e(auth()->user()->kd_pengguna); ?></p>
	                				<?php if(auth()->user()->id_outlet != 0): ?>
	                				<p class="text-dark font-weight-bold" style="font-size: 12px; margin-top: -15px;">Outlet, <?php echo e(auth()->user()->kd_pengguna); ?></p>
	                				<?php endif; ?>
	                				<?php if(auth()->user()->role == 'admin'): ?>
	                				<div class="btn btn-sm font-weight-bold role-success" style="margin-top: -10px;"><?php echo e(auth()->user()->role); ?></div>
	                				<?php else: ?>
	                				<div class="btn btn-sm font-weight-bold role-primary" style="margin-top: -10px;"><?php echo e(auth()->user()->role); ?></div>
	                				<?php endif; ?>
	                			</div>
	                		</div>
	                		<div class="col-md-8">
    						...
    						<table class="table" id="tabel_riwayat_kerja">
							  ...
							  <tbody class="isi_tabel">
							  	...
							  	<tr>
							  		...
							  		<td><?php echo e($transaksi->ket_bayar); ?></td>
							  		<td><?php if($transaksi->status == 'baru'): ?>
...
$('.ubah_password_form').submit(function(e){
	...
	$.ajax({
		url: "<?php echo e(url('/ubah_password/' . auth()->user()->id)); ?>",
		method: "POST",
		data: request,
		contentType: false,
		cache: false,
		...
		success:function(data){
			if(data == 'sukses'){
				swal({
					...
					window.open("<?php echo e(url('/kelola_profil')); ?>", "_self");
				});
			}else{
				gagalPassword();
			}
...
$('.form_edit_identitas').submit(function(e){
	...
	var request = new FormData(this);
	$.ajax({
		url: "<?php echo e(url('/update_profil')); ?>",
		method: "POST",
		data: request,
		contentType: false,
		cache: false,
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('halaman_template', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\SOFTWARE 2\xampp\htdocs\laundry-management-system\resources\views/halaman_profil.blade.php ENDPATH**/ ?>