...
<div class="row page-titles mx-0">
    <div class="col p-md-0">
        <ol class="breadcrumb">
            ...
            <li class="breadcrumb-item active"><a href="<?php echo e(url('/detail_pelanggan_member/' . $id)); ?>">Detail Pelanggan</a></li>
        </ol>
    </div>
</div>
                ...
                <div class="card-body">
                    ...
                    <div class="row mt-3 mb-1">
                        ...
                        <div class="col-12 text-center">
                            <a href="<?php echo e(url('/hapus_pelanggan/'.$id)); ?>" class="btn btn-flat btn-danger font-weight-bold"><i class="fa fa-trash-o"></i>&nbsp;&nbsp;Hapus Pelanggan</a>
                        </div>
                    </div>
                </div>
            </div>  
						...
						<div class="col-md-12">
							<table class="table" id="table-paket-kiloan">
							  ...
							  <tbody class="body-table-kilo">
							  	...
							  	->count();
							  	?>
							  	<?php if($checkout_kilos != 0): ?>
							    <tr class="table-light">
							      <th><?php echo e($transaksi->nama_outlet); ?></th>
							      <td class="text-center"><?php echo e($transaksi->kd_invoice); ?></td>
							      <td class="text-center"><?php echo e(date('d M Y', strtotime($transaksi->tgl_pemberian))); ?></td>
							      <td class="text-center"><?php echo e(date('d M Y', strtotime($transaksi->tgl_selesai))); ?></td>
							      <td>
							      	<?php if($transaksi->status == 'baru'): ?>
                                    <span class="label label-pill label-info text-white"><?php echo e($transaksi->status); ?></span>
                                    <?php elseif($transaksi->status == 'proses'): ?>
                                    <span class="label label-pill label-warning text-white"><?php echo e($transaksi->status); ?></span>
                                    <?php elseif($transaksi->status == 'selesai'): ?>
                                    <span class="label label-pill label-success text-white"><?php echo e($transaksi->status); ?></span>
                                    <?php elseif($transaksi->status == 'diantar'): ?>
                                    <span class="label label-pill label-danger text-white"><?php echo e($transaksi->status); ?></span>
                                    <?php elseif($transaksi->status == 'diambil'): ?>
                                    <span class="label label-pill label-primary text-white"><?php echo e($transaksi->status); ?></span>
                                    <?php endif; ?>
							      </td>
							      <td><button class="btn btn-outline-primary font-weight-bold btn-sm btn-lihat-kilo" data-transaksi="<?php echo e($transaksi->id); ?>" type="button" data-toggle="modal" data-target="#paketKiloModal">Lihat</button></td>
							    </tr>
							    <?php endif; ?>
							...
							<table class="table" id="table-paket-satuan" hidden="">
							  ...
							  <tbody class="body-table-satu">
							  	...
							  	->count();
							  	?>
							  	<?php if($checkout_satus != 0): ?>
							    <tr class="table-light">
							      <th><?php echo e($transaksi->nama_outlet); ?></th>
							      <td class="text-center"><?php echo e($transaksi->kd_invoice); ?></td>
							      <td class="text-center"><?php echo e(date('d M Y', strtotime($transaksi->tgl_pemberian))); ?></td>
							      <td class="text-center">
                                    <?php if($transaksi->tgl_selesai == null): ?>
                                    -
                                    <?php else: ?>
                                    <?php echo e(date('d M Y', strtotime($transaksi->tgl_selesai))); ?>
							      ...
							      <td><button class="btn btn-outline-primary font-weight-bold btn-sm btn-lihat-satu" data-transaksi="<?php echo e($transaksi->id); ?>" type="button" data-toggle="modal" data-target="#paketSatuModal">Lihat</button></td>
							    </tr>
							    <?php endif; ?>
							    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
							  </tbody>
...
<script type="text/javascript">
    <?php if($message = Session::get('batal_pesan')): ?>
    swal(
        "Berhasil!",
        "<?php echo e($message); ?>",
        "success"
	...
	$('.status-select-kilo').change(function() {
		...
		$.ajax({
			...
			success:function(data){
				if(data != ''){
                    window.open("<?php echo e(url('/detail_pelanggan_member/' . $id)); ?>", "_self");
                }
			}
		});
	});
	...
	$(document).on('click', '.btn-lihat-kilo', function(e){
		...
		$.ajax({
			url: "<?php echo e(url('/lihat_paket_kilo_member')); ?>/" + id,
			method: "GET",
	...
	$(document).on('click', '.btn-lihat-satu', function(e){
		...
		var id = $(this).attr('data-transaksi');
		$.ajax({
			url: "<?php echo e(url('/lihat_paket_satu_member')); ?>/" + id,
			method: "GET",
			success:function(response){
                $('.id_pdf').val(response.transaksis.id);
    ...
    $(document).on('click', '.batal-btn-kilo', function(e){
        ...
        var id = $('.id_invoice_kilo').val();
        $.ajax({
            url: "<?php echo e(url('/hapus_pesanan_kilo')); ?>/" + id,
            method: "GET",
            success:function(data){
                window.open("<?php echo e(url('/detail_pelanggan_member/' . $id)); ?>", "_self");
            }
    ...
    $(document).on('click', '.batal-btn-satu', function(e){
        ...
        var id = $('.id_invoice_satu').val();
        $.ajax({
            url: "<?php echo e(url('/hapus_pesanan_satu')); ?>/" + id,
            method: "GET",
            success:function(data){
                window.open("<?php echo e(url('/detail_pelanggan_member/' . $id)); ?>", "_self");
            }
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('halaman_template', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH E:\SOFTWARE 2\xampp\htdocs\laundry-management-system\resources\views/halaman_pelanggan/detail_pelanggan_member.blade.php ENDPATH**/ ?>